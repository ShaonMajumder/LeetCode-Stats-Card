...
export async function HeatmapExtension(generator) {
  const pre_counts = new Promise((resolve) => {
    if (generator.config.site === "us") {
      ...
      lc.graphql({
        ...
        query:
          "query calendar($username: String!, $year: Int) { user: matchedUser(username: $username) { calendar: userCalendar(year: $year) { calendar: submissionCalendar } } }",
      ...
      lc.graphql(
        {
          operationName: "calendar",
          query:
            "query calendar($username: String!, $year: Int) { calendar: userCalendar(userSlug: $username, year: $year) { calendar: submissionCalendar } }",
          variables: { username: generator.config.username },
        },
        "/graphql/noj-go/",
      );
  ...
  return async function Heatmap(generator, data, body) {
    ...
    const blocks = new Item("g", {
      ...
      style: {
        ...
        opacity: generator.config.animation !== false ? 0 : 1,
        animation: generator.config.animation !== false ? "fade_in 1 0.3s 1.9s forwards" : "",
      },
    });
    ...
    extension.children?.push(
      new Item("text", {
        ...
        style: {
          ...
          fill: "var(--text-0)",
          opacity: generator.config.animation !== false ? 0 : 1,
          animation: generator.config.animation !== false ? "fade_in 1 0.3s 2.1s forwards" : "",
          "font-size": "10px",
        },
      }),
      new Item("text", {
